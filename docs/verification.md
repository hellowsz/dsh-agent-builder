...
| 11 | **设计/执行分离全闭环(真 DSH 拼装)** | web API:建任务→起草→候选配置交真 DSH 执行→builder 评审→定稿→资产库 | ✅ 3/3 符合预期,资产上架（2026-08-19） |
| 12 | 一键启动使用 | /api/assets/launch 真起 dsh web | ✅ HTTP 200,CLI 侧同资产可见（2026-08-19） |
| 13 | 资产生命周期:回归集累积/信心分级/运行期回流 | 单测(bank合并/分级/回流读写) + webui 集成 | ✅ 全绿（2026-08-19） |
| 14 | 网络素材真采集 | DuckDuckGo 真搜索 + 真模型提炼 | ✅ 8 片段→1 条真实发票版式素材（2026-08-19） |
...
6. **模型给比较规则写 `factor: 0`**(规则恒不成立,正例必挂)→ 归一化机械丢弃非正 factor,回归测试固化。
